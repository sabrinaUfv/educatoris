...
Para adicionar materiais via API (autenticado como admin):

```bash
# 1. Fazer login e guardar o token
curl -s -X POST http://localhost:3001/api/auth/login \
  -H "Content-Type: application/json" \
  -d '{"email":"admin@educatoris.com","senha":"admin123"}' | grep token

# 2. Adicionar um conteúdo (tema)
curl -X POST http://localhost:3001/api/admin/conteudos \
  -H "Authorization: Bearer SEU_TOKEN" \
  -H "Content-Type: application/json" \
  -d '{"titulo":"Eletrostática","anoEscolar":3,"tema":"Eletricidade"}'

# 3. Adicionar um material ao conteúdo criado
curl -X POST http://localhost:3001/api/admin/materiais \
  -H "Authorization: Bearer SEU_TOKEN" \
  -H "Content-Type: application/json" \
  -d '{
    "conteudoId": 4,
    "titulo": "Aula Pronta: Cargas Elétricas",
    "url": "https://url-do-seu-pdf.com/arquivo.pdf",
    "tipo": "arquivo",
    "extras": {"editavel": false}
  }'
```

Tipos aceitos para `tipo`: `arquivo`, `videoaula`, `laboratorio`

Para `laboratorio`, inclua `"extras": {"remoto": true}` se for laboratório remoto (requer Plano 3).
# educatoris
